...
class EnvLoop:
    ...
    def _step_episode(self, 
        ...
        self.log['timer/episode_total_run_time'] = self.episode_run_time
        self.log['timer/get_action_total_time'] += get_action_time
        self.log['timer/step_env_total_time'] += step_env_time
    ...
    def update_log(self, 
        ...
        # update timer stats
        self.log['timer/get_action_mean_time'] = self.log['timer/get_action_total_time'] / self.env_step_counter
        self.log['timer/step_env_mean_time'] = self.log['timer/step_env_total_time'] / self.env_step_counter
        ...
        self.log['analyzer/mean_summary_table/'] = self.wandb.Table(columns=team_summary_columns)
        for team_id, topn_prob in topn_probs.items():
            row = [team_id, topn_prob]
            team_result = mean_result_by_team[team_id]
            for attr in analyzer.TeamResult.names():
                row.append(getattr(team_result, attr))
            self.log[f'analyzer/mean_summary_table/'].add_data(*row)
